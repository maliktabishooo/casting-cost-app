...
def total_costs(p):
    costs = {
        'Direct Mat': direct_material_cost(p),
        'Indirect Mat': indirect_material_cost(p),
        'Labor': labor_cost(p),
...
with st.form("cost_form"):
    ...
    with col2:
        density = st.number_input("Density (kg/m³)", value=float(METAL_DENSITIES[metal]), step=100.0, key='density')
        st.button("Sync Density", on_click=lambda: st.session_state.update(density=float(METAL_DENSITIES[st.session_state.metal])))
...
if submitted:
    try:
        fe = FURNACE_EFFICIENCY[st.session_state.furnace]
        p = {k: st.session_state[k] for k in [
            ...
            'f_y', 'other_energy_rate', 'tooling_index', 'quantity',
